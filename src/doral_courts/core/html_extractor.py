...
class Court:
    ...
    def time_slot(self) -> str:
        ...
        available_count = sum(
            1 for slot in self.time_slots if slot.status == "Available"
        )
...
class CourtAvailabilityHTMLExtractor:
    ...
    def parse_court_data(self, soup: BeautifulSoup) -> List[Court]:
        ...
        for table_index, court_table in enumerate(court_tables):
            ...
            if not tbody:
                logger.debug(
                    "Could not find tbody in court table %d, skipping", table_index + 1
                )
                continue
            ...
            court_rows = [
                row for row in all_rows if not row.find("td", class_="cart-blocks")
            ]

            logger.debug(
                "Found %d court data rows in table %d", len(court_rows), table_index + 1
            )

            for i, row in enumerate(court_rows):
                logger.debug(
                    "Processing court row %d/%d in table %d",
                    i + 1,
                    len(court_rows),
                    table_index + 1,
                )
                try:
                    ...
                    if len(cells) < 4:
                        logger.debug(
                            "Row %d has insufficient cells (%d), skipping",
                            i + 1,
                            len(cells),
                        )
                        continue
                    ...
                    name = (
                        name_cell.get_text(strip=True) if name_cell else "Unknown Court"
                    )

                    # Extract location description
                    location_cell = row.find(
                        "td", {"data-title": "Location Description"}
                    )
                    location = (
                        location_cell.get_text(strip=True)
                        if location_cell
                    ...
                    capacity = (
                        capacity_cell.get_text(strip=True) if capacity_cell else "N/A"
                    )
                    ...
                    courts.append(court)
                    logger.debug(
                        "Successfully parsed court: %s (%s) at %s on %s",
                        name,
                        sport_type,
                        location,
                        date,
                    ...
                    logger.warning(
                        "Error parsing court row %d in table %d: %s",
                        i + 1,
                        table_index + 1,
                        e,
                    )
    ...
    def _extract_time_slots(self, court_row) -> List[TimeSlot]:
        ...
        for button in cart_buttons:
            try:
                ...
                is_available = (
                    "success" in button_classes and "Book Now" in button_tooltip
                )
                ...
                logger.debug(
                    "Parsed time slot: %s - %s (%s)", start_time, end_time, status
                )
